...
def _record_sync_failure(get_db, account, error):
    """Persist failures so an admin can diagnose syncs that fail before upsert."""
    ...
    def meta_sync_status():
        ...
        try:
            ...
            for key, table in (
                ...
                ('adsets', 'meta_adsets'),
                ('ads', 'meta_ads'),
                ('insights', 'meta_ad_insights'),
            ):
                cur.execute('SELECT COUNT(*) FROM ' + table)
                counts[key] = cur.fetchone()[0]
            cur.execute("""
                SELECT status,meta_account_id,campaigns_count,adsets_count,ads_count,
                       insights_count,error_message,started_at,finished_at
                FROM meta_sync_logs ORDER BY id DESC LIMIT 1
            """)
            row = cur.fetchone()
            cur.close()
            latest = None
            if row:
                latest = {
                    'status': row[0], 'account': row[1], 'campaigns': row[2],
                    'adsets': row[3], 'ads': row[4], 'insights': row[5],
                    'error': row[6],
